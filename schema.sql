...
CREATE TABLE `resource` (
  ...
  `public_ip_enabled` tinyint(1) DEFAULT '0',
  `public_ip` varchar(45) DEFAULT NULL,
  `private_ip` varchar(45) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `asset_no` (`asset_no`),
  CONSTRAINT `resource_ibfk_1` FOREIGN KEY (`asset_no`) REFERENCES `inventory` (`asset_no`) ON DELETE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `storage` (
  ...
  CONSTRAINT `storage_ibfk_1` FOREIGN KEY (`resource_id`) REFERENCES `resource` (`id`) ON DELETE CASCADE
