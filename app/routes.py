...
def add_inventory():
    if request.method == 'POST':
        ...
        db.session.add(AuditLog(action='Added Inventory', entity_type='inventory', entity_id=inventory.id, performed_by=session.get('user')))
        db.session.commit()
        return redirect(url_for('routes.dashboard'))
...
def add_resource(asset_no):
    if request.method == 'POST':
        ...
        db.session.add(AuditLog(action='Added Resource', entity_type='resource', entity_id=resource.id, performed_by=session.get('user')))
